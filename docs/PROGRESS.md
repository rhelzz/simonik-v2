...
**Deduplikasi saat planning: 13 butir → 10 fase.** Enam butir ternyata tiga fitur yang sama, diminta dua kali (sekali "role admin", sekali "role guru pembimbing") — padahal `/monitoring/absen` adalah **satu halaman yang sama** untuk kedua role (`routes/web.php:279-282`, satu grup middleware) dengan data yang sudah dibatasi `ScopesStudentsByRole`. Dibangun sekali, di-gate lewat prop `can`. Dua permintaan reset (jurusan/kelas/industri vs tanggal/semua-murid/beberapa-murid) juga digabung jadi satu modal — dua tombol reset di satu halaman adalah cara tercepat operator menghapus hal yang salah.

**Fase 21 — hapus tabel "Siswa terbaru" (dashboard admin).** Dihapus di **dua sisi**: prop `recentStudents` di `DashboardController::adminDashboard()` dan blok `<RecentStudentsTable>` di `dashboard.tsx`. Yang **tidak** ikut dihapus meski menggoda: method `presentStudents()` (masih dipakai `kaprogDashboard()` untuk `notStartedStudents` dan `staffDashboard()`) serta komponen `RecentStudentsTable` (masih dipakai `dashboard-staff.tsx` & `dashboard-kaprog.tsx`) — "hapus fitur X" tidak berarti "hapus semua kode yang dipakai fitur X". Assertion barunya `->missing('recentStudents')`, bukan sekadar menghapus assertion lama, supaya propnya tidak diam-diam kembali.

**Fase 22 — card Rate absensi di Data Absen + hapus grid gap.**
- Rumus rate (`participation`/`rates`/`weekdaysBetween`/`rate`) tadinya `private` di `DashboardController`, jadi tidak bisa dipakai halaman lain. Diekstrak ke trait baru `Concerns/SummarizesParticipation` (pemakai: `DashboardController` 4 method + `AttendanceMonitorController` — memenuhi syarat "≥2 pemakai hari ini"). Menyalin rumusnya ditolak: dua salinan akan berbeda diam-diam, lalu dashboard dan Data Absen menampilkan angka berbeda untuk hal yang sama.
- **Tabrakan trait yang tidak tertangkap saat planning:** `SummarizesStudentPerformance` (sudah dipakai `AttendanceMonitorController`) **punya `rate()` sendiri** dengan signature berbeda — memasang trait kedua yang juga bernama `rate()` = fatal error `Trait method collision`. Method di trait baru diberi nama berbeda (`participationRate()`, `participationRates()`); isinya tetap pindahan murni.
- Rate di Data Absen **dibatasi cakupan role** (`scopedStudents()`), bukan seluruh sekolah — kalau tidak, guru melihat angka rate sekolah di halaman yang seluruh isinya siswanya sendiri. Test membuktikan: guru 100%, admin 25%, data yang sama.
- Grid kartu jurusan: `gap-3` → `gap-0` sesuai permintaan literal. Konsekuensi yang harus ikut ditangani — tiap kartu punya `border` sendiri, jadi kartu bersentuhan menghasilkan garis **dobel 2px**. Diperbaiki dengan memindahkan pemisah ke wadah (`border-t border-l` di grid + `border-r border-b` di kartu), menghasilkan seam 1px konsisten dengan sudut luar tetap membulat.
- ✅ **Jaring pengaman ekstraksi trait: seluruh `DashboardTest.php` hijau tanpa satu pun perubahan pada file test itu** — bukti pemindahannya murni.
- ✅ Pint + PHPStan 0 error + 21/21 passed (221 assertions) + `tsc` + ESLint/Prettier lolos.

**Fase 23 — tabel murid sudah & belum presensi.** Panel "Presensi harian" di `/monitoring/absen`, di atas kartu jurusan (pertanyaan harian dulu, penelusuran belakangan).
- **Definisi "sudah presensi" = ADANYA baris `attendances` pada tanggal itu — bukan `status = 'hadir'`.** Siswa sakit/izin/libur (hasil approval) sudah punya baris, jadi tidak boleh muncul di daftar "belum". Kalau ini salah, siswa sakit ber-approval lengkap akan dipresensikan paksa lewat Fase 24 dan status sakitnya tertimpa — ada test khususnya.
- Populasi = siswa `status_pkl = 'proses'` dalam cakupan role, `whereNotNull('user_id')` (siswa tanpa akun tidak pernah bisa absen). Satu closure `$activeStudents` dipakai roster **dan** perhitungan rate Fase 22 — satu definisi "siswa aktif dalam cakupan", bukan dua.
- Satu tabel + tab `Belum (N)` / `Sudah (N)`, bukan dua tabel bersanding: dua paginasi independen di satu layar membingungkan dan di HP menumpuk jadi layar sangat panjang. Kedua angka tetap terlihat sekaligus lewat label tab. Default tab = **Belum** (daftar yang butuh tindakan).
- Kueri: `whereHas` / `whereDoesntHave` (satu SQL, dipaginasi database) — bukan ambil semua lalu selisihkan di PHP. `$belum` dihitung dengan pengurangan, menghemat satu `NOT EXISTS` per muat halaman.
- **Temuan PHPStan yang mencegah bug:** satu closure `$onDate` dipakai tiga kali, tapi `whereHas()` mengirim `Builder` sedangkan `with()` mengirim `Relation` — tipe `Builder` saja ditolak PHPStan. Diperbaiki dengan tipe union `Builder|Relation`, bukan dengan menduplikasi closure (duplikasi = kriteria hitung & saring bisa berbeda diam-diam).
- Eager-load berkondisi `'users.attendances' => $onDate` — tanpa itu tabel 15 baris = 15 kueri tambahan.
- Empty state tab "belum" dibedakan sebagai **keadaan sukses** ("Semua murid sudah presensi pada tanggal ini"), bukan "tidak ada data".
- ✅ Pint + PHPStan 0 error + 17/17 `AttendanceMonitorTest` + 44/44 suite bertetangga (Dashboard/Attendance/JournalMonitor/KaprogScope/InboxApproval) + `tsc` + ESLint/Prettier lolos.

**Fase 24 — presensi diwakilkan guru pembimbing / admin.** Tombol + modal menempel di panel Fase 23; daftar sumbernya tab "Belum", jadi secara alami tidak mungkin memilih murid yang sudah absen.
- **`AttendanceController::checkIn()` tidak dipakai ulang dan sengaja tidak dilonggarkan.** Seluruh jalur itu mensyaratkan foto & GPS (gerbang akurasi, geofencing radius, heuristik lompatan >100km, `is_suspect`); melonggarkannya akan membuat setiap pengaman anti-titip-absen di jalur siswa jadi opsional. Yang diikuti justru preseden `ApproveRequest::handle()` yang memang sudah membuat baris absen tanpa foto/GPS atas nama orang lain.
- **`create()`, BUKAN `updateOrCreate()`** — beda disengaja dari preseden itu. Approval memang berwenang mengubah status; presensi diwakilkan tidak. Menimpa berarti menghapus bukti foto+GPS absen mandiri siswa atau membatalkan status sakit yang sudah lolos approval. Yang sudah punya data **dilewati dan dilaporkan** (`"12 dipresensikan. 2 dilewati karena sudah punya data absen."`), bukan ditindih diam-diam.
- `mode = 'proxy'` (kolom sudah ada, nol migrasi) + `description` berisi nama & role pelakunya — baris tanpa foto/GPS yang terlihat identik dengan absen mandiri adalah kebohongan data.
- `is_late` **tetap dihitung** dari waktu yang diketik vs `industries.jam_masuk`; kalau tidak, fitur ini jadi jalan pintas menghapus keterlambatan.
- `image`/`latitude`/`longitude`/`gps_accuracy` dibiarkan `null` — koordinat industri **tidak** diisikan sebagai "perkiraan" (itu memalsukan bukti lokasi). Ada testnya.
- `BadgeAwarder` sengaja **tidak** dipanggil: badge adalah penghargaan atas perilaku siswa, sedangkan baris ini dibuat guru.
- **Celah nyata yang ditemukan saat grep pembaca kolom `mode`:** `AttendanceController::checkOut()` menggerbang geofencing dengan `$today->mode === 'wfo'`, sehingga absen bermode `proxy` akan **melewati geofencing saat pulang**. Diperbaiki jadi `in_array($today->mode, ['wfo','proxy'], true)` — ditulis eksplisit, **bukan** `!== 'wfa'`, agar baris lama bermode `NULL` tidak ikut berubah perilakunya. Tidak ada `match()` pada `mode` di mana pun, jadi nilai ketiga aman untuk pembaca lain.
- Galat form memakai `text-red-500` mengikuti konvensi yang sudah ada (`student-form.tsx`, `leave-requests/index.tsx`) — token `danger` belum ada di `@theme` dan fase ini bukan tempat memperkenalkannya.
- ✅ Pint + PHPStan 0 error + 11/11 `ProxyAttendanceTest` + **79/79** suite regresi (Attendance/AttendanceMonitor/BadgeAwarder/StreakCalculator/ApproveRequest/InboxApproval) + `tsc` + ESLint/Prettier lolos.

**Fase 27 — "tidak presensi = Alpha".** Menutup rantai halaman Data Absen.
- **Alpha diturunkan saat dibaca, bukan disimpan sebagai baris** — `app/Support/AttendanceStatus.php`. Alasan penentunya bukan "lebih sedikit kode" tapi **koreksi terlambat**: approval sakit sering datang 2-3 hari setelah tanggalnya dan Fase 24 seluruhnya tentang mengisi presensi yang terlewat. Dengan baris `alpha` tersimpan, setiap jalur penulisan absen harus tahu cara membersihkannya; dengan turunan, tidak ada yang perlu dibersihkan. Bonus: nol cron (repo ini tidak punya scheduler sama sekali — `routes/console.php` hanya berisi `inspire`), nol backfill, langsung benar untuk seluruh riwayat.
- Definisi Alpha = tidak ada baris absen **AND** tanggal sudah lewat **AND** `status_pkl = 'proses'` **AND** bukan akhir pekan **AND** dalam periode PKL. **Hari berjalan tidak pernah Alpha** — tanpa aturan itu seluruh sekolah tampil "Alpha" tiap pagi jam 7 sampai mereka absen satu per satu, dan panelnya berhenti dibaca orang. Tidak ada ambang jam (mis. "Alpha setelah pukul 10"): tampilan yang berubah arti di tengah hari tanpa ada yang melakukan apa pun hanya membingungkan.
- **Satu kamus label untuk seluruh aplikasi** (`AttendanceStatus::LABELS`) — konstanta `STATUS_LABELS` lokal milik Fase 23 dilebur ke sana supaya tidak ada dua kamus status di dua tempat.
- **Penyesuaian dari rencana, dicatat di `docs/v2.4/27-*.md` §3.7a:** akhir pekan disaring di kueri (properti tanggal, satu cek), tapi batas periode PKL per-siswa ditegakkan lewat **label** ("Tidak dihitung"), bukan filter kueri — menyatakan `COALESCE(students.pkl_start, pkl_periods.start_period)` di SQL butuh 4 cabang `OR` bersarang di dalam kueri yang sudah memuat `whereDoesntHave`. Tujuan aslinya tetap tercapai (tidak ada siswa ditandai Alpha secara tidak adil); yang tidak tercapai: barisnya masih ikut terhitung di angka `summary.belum`.
- **Batasan yang diakui terbuka:** libur nasional & cuti bersama **tidak dikenali** (tidak ada kalender hari libur di sistem), jadi 17 Agustus akan tampil sebagai Alpha massal. Jalan keluar yang sudah tersedia hari ini tanpa modul baru: ajukan **Libur** lewat `LeaveRequest` — approval-nya membuat baris `status = 'libur'` yang langsung mematikan Alpha.
- Catatan di UI: *"Alpha dihitung otomatis dari ketiadaan data presensi pada hari kerja"* — Alpha adalah kesimpulan yang ditarik sistem, bukan sanksi yang diketik seseorang; dinyatakan agar tidak jadi sengketa dengan siswa/orang tua.
- ✅ Pint + PHPStan 0 error + 11/11 `AttendanceStatusTest` (unit, tanpa DB, 135ms) + 21/21 `AttendanceMonitorTest` + **88/88** suite regresi (termasuk Rapor & Certificate yang ikut membaca rekap absen) + `tsc` + ESLint/Prettier lolos. Semua test memakai `Carbon::setTestNow()` — tanpa itu suite lulus hari Selasa dan gagal hari Sabtu.

**Fase 19 — reset data absen (DESTRUKTIF).** Satu tombol + satu modal dengan lima sumbu filter (jurusan / kelas / industri / rentang tanggal / murid tertentu), bukan dua tombol reset di satu halaman — permintaan aslinya datang dua kali dengan sumbu berbeda, dan dua tombol hapus berdampingan adalah cara tercepat operator menghapus hal yang salah.
- **`ResetStudentRecords` (action)** menerima builder siswa yang SUDAH ter-scope dari pemanggil; ia tidak pernah membangun `Student::query()` sendiri. Kalau boleh, suatu hari ada yang memanggilnya tanpa scoping dan seorang guru bisa menghapus data satu sekolah. Murid tertentu disaring **lewat** builder itu, bukan langsung ke tabel absen — inilah yang membuat ID murid sekolah lain dari devtools menghasilkan 0 baris.
- **`count()` dan `handle()` memakai SATU pembangun kueri privat** — pratinjau yang menghitung himpunan berbeda dari yang dihapus adalah bug paling berbahaya di fitur ini. Ada test yang membandingkan angka pratinjau dengan selisih baris sungguhan.
- **Approval yatim dicegah:** `Approval` polimorfik, **tidak** punya FK ke `attendances`, dan `->delete()` pada query builder tidak memicu event model — jadi tidak ada cascade apa pun. Tanpa pembersihan eksplisit, Inbox Persetujuan FATAL saat `ApprovalController::index()` memanggil `$approvable->users->name` pada approvable null. Keduanya dalam satu `DB::transaction` (separuh terhapus lebih buruk daripada gagal), dan ada test yang memastikan approval milik siswa lain selamat.
- **Password:** aturan `current_password` bawaan Laravel. Middleware `password.confirm` **ditolak** — session-nya 3 jam, jadi reset kedua dalam 3 jam tidak akan meminta password lagi; untuk aksi yang menghapus ribuan baris permanen, "sudah dikonfirmasi 2 jam lalu" bukan konfirmasi.
- **Pratinjau server-side** (`POST .../pratinjau` → JSON) menampilkan `N baris akan dihapus` sebelum tombol aktif. JSON, bukan Inertia render — satu-satunya pengecualian "tanpa API terpisah" di proyek ini, karena pratinjau dipanggil berkali-kali di dalam modal yang sedang terbuka. Flash menyebut **angka nyata**, bukan "berhasil".
- **Kesalahan rencana yang ketahuan saat implementasi:** rencana Fase 19 & 23 mengasumsikan `students.user_id` nullable ("siswa tanpa akun") dan memasang guard `whereNotNull('user_id')` di tiga tempat. Ternyata kolom itu **NOT NULL** (`2025_01_01_000009_create_students_table.php:16`) — yang dibuat nullable di Fase 12 dulu adalah `parents.user_id`. Ketiga guard dihapus sebagai kode mati; faktanya dicatat di komentar agar tidak "diperbaiki" lagi nanti.
- **ESLint menangkap `setState` langsung di badan `useEffect`** pada modal pratinjau. Diperbaiki dengan menyimpan hasil pratinjau BERSAMA kriteria yang menghasilkannya (`{key, count}`), sehingga "sedang menghitung" **diturunkan** (`result.key !== serialised`) alih-alih jadi state — efek sampingan yang bagus: angka basi tidak mungkin tampil sebagai angka yang berlaku.
- ✅ **`php artisan test` PENUH: 487/487 passed, 2114 assertions.** PHPStan 0 error di seluruh `app/`. Pint + `tsc` + ESLint/Prettier lolos. 12/12 `ResetAttendanceTest`.

**Fase 20 — reset data jurnal (DESTRUKTIF).** Menyambung ke `ResetStudentRecords` yang lahir di Fase 19 — **nol baris logika baru di action**, tinggal dipanggil dengan `Activity::class`.
- **Diperiksa langsung, bukan percaya dokumen rencana:** `Activity` hanya punya `users()` BelongsTo — **tidak ada relasi morph** seperti `Attendance::approval()`, jadi tidak ada pembersihan tambahan yang perlu dilakukan.
- **Form Request digabung, bukan digandakan.** Rencana §4.1 sengaja menggantung keputusannya sampai kodenya terlihat; ternyata aturannya identik, jadi `ResetAttendanceRequest`/`PreviewResetAttendanceRequest` **diganti nama** jadi `ResetRecordsRequest`/`PreviewResetRecordsRequest` dan dipakai kedua modul — bukan sepasang kelas kembar.
- **Modal frontend DISALIN, bukan digeneralisasi.** Komponen generik `<ResetModal entity=…>` butuh >=6 prop (judul, dua endpoint, kata benda, dua daftar opsi) demi menghemat JSX yang tidak punya logika. Dua berkas yang jujur lebih mudah dibaca daripada satu abstraksi berparameter enam; alasannya ditulis sebagai komentar `ponytail:` beserta syarat penggabungan (kalau modul KETIGA butuh reset yang sama).
- Filter industri untuk jurnal jalan **tanpa satu baris kode tambahan**: `activities` tidak punya kolom industri, tapi penyaringan memang terjadi di sisi siswa (`students.industri_id`).
- **Test penjaga kesalahan salin-tempel:** `test_reset_journal_does_not_touch_attendances` **dan** kebalikannya `test_reset_attendance_does_not_touch_journals`. Fase ini secara harfiah menyalin Fase 19, dan satu `class-string` yang lupa diganti akan menghapus modul yang salah tanpa gejala apa pun sampai ada yang membuka modul itu.
- Badge yang sudah diraih **tidak dicabut** (keputusan user) — ada testnya; teks di modal menyatakan ini ke operator sebelum ia menekan tombol ("Streak siswa akan kembali 0; badge yang sudah diraih tetap").
- ✅ **`php artisan test` PENUH: 495/495 passed, 2143 assertions.** PHPStan 0 error di seluruh `app/`. Pint + `tsc` + Prettier (seluruh `resources/js/`) lolos. 8/8 `ResetJournalTest`.

**Fase 26 — Sakit & Izin jadi SATU tahap approval.** Dikonfirmasi user dua kali: *"satu tahap saja"*, lalu *"kalau izin itu jadi gausah ada orang tua sama sekali berarti"*. Jadi tahap Orang Tua dihapus sepenuhnya untuk **kedua** jenis pengajuan.
- **Blokade lama dihapus, bukan dipersempit.** `SakitIzinController::store()` dulu menolak siswa yang belum menautkan akun Orang Tua; sekarang tidak ada syarat itu sama sekali. Flash sukses ikut diubah — teks lama menyebut "menunggu persetujuan Orang Tua", dan kalau tidak diubah siswa akan menunggu pihak yang tidak lagi terlibat.
- **`scopeForUserQueue()` MENYUSUT, bukan bertambah cabang.** Blok `if (orangtua) { whereNotExists… } else { whereExists first_app approved… }` dibuang seluruhnya; cabang `SakitIzin` kini diperlakukan seragam dengan `LeaveRequest` & `Attendance` di dalam satu penjaga `ELIGIBLE_ROLES`. Method paling rumit di repo ini jadi lebih pendek — bukti bahwa "satu tahap" memang penyederhanaan, bukan penambahan aturan.
- **Cabang `index === 1` DIPERTAHANKAN** khusus untuk pengajuan lama yang terlanjur dua tahap (tahap 1 approved, tahap 2 pending). Menghapusnya membuat pengajuan yang sedang berjalan di produksi tidak bisa dituntaskan siapa pun, tanpa gejala sampai ada guru yang mencoba. Kueri SQL untuk membuktikan kapan cabang itu aman dihapus ditulis di komentar. Ada testnya (`test_legacy_two_stage_sakit_can_still_be_completed`).
- **Pembuatan baris presensi diekstrak** jadi `ApproveRequest::recordSakitIzin()` — dipakai alur satu-tahap (baru) dan dua-tahap (data lama), supaya tidak ada dua salinan yang bisa berbeda diam-diam. `getRawOriginal('bukti')` dipertahankan dan diberi komentar: accessor `SakitIzin::bukti()` mengubah nilainya jadi URL `asset()`, dan menyimpan URL ke kolom `image` akan merusak foto bukti di rekap absen. Ada testnya.
- **Inbox Persetujuan dicabut dari role `orangtua` di 4 berkas** (`routes/web.php`, `ApprovalController::index`, `HandleInertiaRequests::pendingApprovalsCount`, `nav.ts`) + cabang `orangtua` di `scopeForUserQueue` dihapus sebagai kode mati. Alasannya: cabang `SakitIzin` adalah SATU-SATUNYA yang dulu mengisi antrean orang tua (cabang `LeaveRequest`/`Attendance` dibungkus `ELIGIBLE_ROLES` yang tidak memuat `orangtua`), jadi menyisakan menunya = halaman yang selamanya kosong dan terlihat seperti aplikasi rusak.
- **UI siswa:** panel dua tahap ("Tahap 1: Orang Tua" → "Tahap 2: Industri/Guru") diganti satu blok "Persetujuan Guru Pembimbing". Pengajuan lama yang punya dua baris approval tetap terbaca — yang ditampilkan adalah keputusan **terakhir**.
- **Tiga test lama yang mengunci aturan versi lama ditulis ulang, bukan dihapus:** `test_student_must_have_parent_to_submit_sakit_izin` → dibalik jadi `test_student_without_parent_can_submit_sakit_izin` (+ varian izin), `test_parent_and_industry_multi_stage_approval_flow` → `test_single_stage_approval_flow`, `test_parent_can_only_see_stage_1_sakit_izin` → `test_parent_can_no_longer_access_approval_inbox`. Ditambah `test_bukti_is_still_required` dan `test_sakit_appears_in_guru_inbox_immediately`.
...
7. **Perluas halaman impor ke entitas lain** bila operator menyukainya. Halamannya sudah generik, jadi biayanya kecil per entitas. Jangan dikerjakan borongan sebelum halaman siswa terbukti dipakai.
8. **Saran kandidat relasi terdekat** (`levenshtein`, "maksud Anda: XI RPL 1?") bila salah ketik nama relasi ternyata masih sering terjadi.
